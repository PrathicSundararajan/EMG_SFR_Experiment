...
volatile unsigned long last_calls[sizeof(adc_pins)];
...
void setup()
{
  ...
  ADMUX = bit (REFS0) | bit (ADLAR) | (adc_pins[0] & 7);
...
{ 
  ...
  adc_now = ADC>>6;// current ADC value
  //For 8-bit resolution (for operating with a prescaler as low as 16)
  //adc_now = ADCH;// current ADC value
 
  //FOR SINGLE INPUT (SIGNAL)
  serial_string += String (adc_now) + "," + String (current_time) + "\n";
  last_calls[0] = current_time;
  
  //FOR MULTIPLE INPUTS (SIGNALS)
  /*adc_pin_counter = (adc_pin_counter + 1) % adc_pins_count;
  if (adc_pin_counter == 0)
  {
  ...
  //Increment Analog Read Pin
  ADMUX = bit (REFS0) | bit (ADLAR) | (adc_pins[adc_pin_counter] & 7);*/
}
